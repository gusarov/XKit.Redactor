...
- **A `Label` must be something the code chose** — a key name, a rule name, a type name — never anything read out of the value, or the mode's promise that nothing of the value survives quietly stops being true. Rule labels are constants on `CredentialRedactor.Rule.Name` and string literals in `EntropyRedactor`; resolution order is `options.Label ?? options.Key ?? <rule name>`, falling back to `Erase`.
