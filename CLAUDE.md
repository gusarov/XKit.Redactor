...
- **Every mode decision goes through `RedactorOptions.Hide`.** It is the one place the `Erase`/`Mask`/`Label` switch exists, so the modes cannot drift between `CredentialRedactor`, `EntropyRedactor` and the string helpers — and a fourth mode is one edit. Do not reintroduce a per-redactor `Hide`.
- **Labels are caller-driven: `options.Label ?? options.Key`, and no redactor authors one.** Do not add rule names, detector reasons or anything else the package invents — a rule preserves the structure around what it hides, so an invented label could only restate what is already on screen (`mongodb://●●●userinfo●●●@host` adds nothing to `://…@`). A label earns its place only where the *whole* value is hidden. Keeping the package out of the label business also leaves exactly one place — a call site — where "never derive a label from the value" can be broken.
- `RedactorException` deliberately has no constructor without an inner exception, and `ToString()` is a *sealed* override with no opt-out. Both are guarantees, not defaults — `RedactorExceptionTests` asserts each by reflection so removing one fails rather than silently weakening the type. The owner's `exception-handling` skill is where the reasoning lives.
